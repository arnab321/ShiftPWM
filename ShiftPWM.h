...
static inline void pwm_output_one_pin(unsigned char counter, unsigned char * ledPtr){
	GPOC = (1 << ShiftPWM_clockPin);
	if(ShiftPWM_invertOutputs)
		((*(ledPtr)<=counter)? GPOS : GPOC) = (1 << ShiftPWM_dataPin);
	else
		((*(ledPtr)>counter)? GPOS : GPOC) = (1 << ShiftPWM_dataPin);
	GPOS = (1 << ShiftPWM_clockPin);
}
...
static inline void ShiftPWM_handleInterrupt(void){
	...
	//Use port manipulation to send out all bits
		...
		for(unsigned char i = ShiftPWM.m_amountOfRegisters; i>0;--i){   // do one shift register at a time. This unrolls the loop for extra speed
			...
			pwm_output_one_pin(counter, --ledPtr);  // This takes 12 or 13 clockcycles
			pwm_output_one_pin(counter, --ledPtr);
			pwm_output_one_pin(counter, --ledPtr);
			pwm_output_one_pin(counter, --ledPtr);
			pwm_output_one_pin(counter, --ledPtr);
			pwm_output_one_pin(counter, --ledPtr);
...
#ifdef ESP8266
extern void ShiftPWM_timer_init(){
	ticks = round((float) F_CPU/((float) ShiftPWM.m_ledFrequency*((float) ShiftPWM.m_maxBrightness+1)))-1;
	
	noInterrupts();
	timer0_isr_init();
	timer0_attachInterrupt(ShiftPWM_handleInterrupt);
	timer0_write(ESP.getCycleCount() + ticks);
	interrupts();
}
